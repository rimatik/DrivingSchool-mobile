...
/**
 * Created by Computer on 15/08/2014.
 */
public class DbHelperZnakoviIzricitihNaredbi extends SQLiteOpenHelper {
    ...
    public DbHelperZnakoviIzricitihNaredbi(Context context) {
        super(context, DATABASE_NAME, null, DATABASE_VERSION);
    }
    @Override
    public void onCreate(SQLiteDatabase db) {
        dbase=db;
        String sql = "CREATE TABLE IF NOT EXISTS " + TABLE_ZNAK_IZRICITIH_NAREDBI + " ( "
...
// Drop older table if existed
        db.execSQL("DROP TABLE IF EXISTS " + TABLE_ZNAK_IZRICITIH_NAREDBI);
// Create tables again
        onCreate(db);
    }
    // Adding new question
...
// Inserting Row
        dbase.insert(TABLE_ZNAK_IZRICITIH_NAREDBI, null, values);
    }
    public List<Znak> getAllSigns() {
        List<Znak> signList = new ArrayList<Znak>();
// Select All Query
        String selectQuery = "SELECT * FROM " + TABLE_ZNAK_IZRICITIH_NAREDBI;
